...
#define ADS1115_QUEUE_NUM_ITEMS 5
#define ADS1115_QUEUE_ITEM_SIZE sizeof(float)

static uint8_t ads1115_queue_buf[ADS1115_QUEUE_NUM_ITEMS * ADS1115_QUEUE_ITEM_SIZE];
...
TASK(ads1115_writer, TASK_STACK_256) {
  ...
  while (true) {
    ...
    StatusCode status = queue_send(&ads1115_data_queue, &voltage, ADS1115_SAMPLING_PERIOD_MS);
...
TASK(ads1115_reader, TASK_STACK_256) {
  ...
  while (true) {
    float received = 0.0f;
    StatusCode status = queue_receive(&ads1115_data_queue, &received, ADS1115_SAMPLING_PERIOD_MS);
    if (status == STATUS_CODE_OK) {
