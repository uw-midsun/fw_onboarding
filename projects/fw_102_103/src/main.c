...
static uint8_t ads1115_queue_buf[ITEMS*SIZE];
...
TASK(ads1115_writer, TASK_STACK_256) {
  ...
  float data;
  while(true) {
    ads1115_read_converted(&ads1115_cfg, ADS1115_CHANNEL_0, &data);
    StatusCode status = queue_send(&ads1115_data_queue, &data, ADS1115_SAMPLING_PERIOD_MS);
...
TASK(ads1115_reader, TASK_STACK_256) {
  ...
  float data;
  while(true) {
    StatusCode book = queue_receive(&ads1115_data_queue, &data, ADS1115_SAMPLING_PERIOD_MS);
    if (book != STATUS_CODE_OK) {
...
int main() {
  ...
  tasks_init_task(ads1115_reader, TASK_PRIORITY(1), NULL);
  /* --------------------- FW103 END --------------------- */
