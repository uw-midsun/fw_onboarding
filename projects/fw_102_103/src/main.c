...
static uint8_t ads1115_queue_storage[NUM_ITEMS * sizeof(float)];
...
TASK(blinky, TASK_STACK_256) {
  ...
  while (true) {
    LOG_DEBUG("Blink - State: %s\n", gpio_get_state(&blinky_gpio) == GPIO_STATE_LOW ? "OFF" : "ON");
...
TASK(ads1115_writer, TASK_STACK_256) {
  ...
  float reading;
  while (true) {
    StatusCode status = ads1115_read_converted(&ads1115_cfg, ADS1115_CHANNEL_0, &reading);
    if (status == STATUS_CODE_OK) {
      status = queue_send(&ads1115_data_queue, &reading, ADS1115_SAMPLING_PERIOD_MS);
    }
    if (status == STATUS_CODE_OK) {
