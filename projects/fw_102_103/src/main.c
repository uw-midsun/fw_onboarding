...
TASK(blinky, TASK_STACK_256) {
  ...
  while(true) {
    ...
    LOG_DEBUG("Blinky! State: %s\n", gpio_get_state(&blinky_gpio) == 1 ? "On" : "Off");
...
TASK(ads1115_writer, TASK_STACK_256) {
  ...
  while(true) {
    ...
    if (status != STATUS_CODE_OK) {
      ...
      queue_send(&ads1115_data_queue, &voltage, 1000);
...
TASK(ads1115_reader, TASK_STACK_256) {
  ...
  float received = 0;
  while(true) {
    StatusCode status = ads1115_read_converted(&ads1115_cfg, ADS1115_CHANNEL_0, &received);

    if (status != STATUS_CODE_OK) {
      LOG_DEBUG("Read from queue failed");
    }
    else {
      queue_receive(&ads1115_data_queue, &received, 1000);
...
int main() {
  ...
  tasks_init_task(blinky, TASK_PRIORITY(0), NULL);
