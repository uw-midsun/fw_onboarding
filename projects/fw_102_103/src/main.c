...
static ADS1115_Config ads1115_cfg = { .i2c_port = ADS1115_I2C_PORT };
static uint8_t queue_storage[ADS_ITEMS * sizeof(float)];
...
TASK(ads1115_writer, TASK_STACK_256) {
  ...
  while (ads1115_init(&ads1115_cfg, ADS1115_ADDR_GND, &ready_pin) != STATUS_CODE_OK) {
    LOG_DEBUG("adc initialization failed\n");
...
TASK(ads1115_reader, TASK_STACK_256) {
  ...
  while (true) {
    float voltage;
    if (queue_receive(&ads1115_data_queue, &voltage, 1000U) == STATUS_CODE_OK) {
