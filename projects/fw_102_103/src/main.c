...
TASK(blinky, TASK_STACK_256) {
  ...
  while (true){
    ...
    LOG_DEBUG("blinky %s\n", state); // TODO: include the state of the LED in the output.
...
TASK(ads1115_writer, TASK_STACK_256) {
  ...
  float converted_voltage = 0.0;

  while (true){
    ads1115_read_converted(&ads1115_cfg, ADS1115_CHANNEL_1, &converted_voltage);
    
    StatusCode status = queue_send(&ads1115_data_queue, &converted_voltage, 1000);
...
TASK(ads1115_reader, TASK_STACK_256) {
  ...
  while (true){
    StatusCode status = queue_receive(&ads1115_data_queue, &data, 1000);
    if (status == STATUS_CODE_OK){
      LOG_DEBUG("Value from queue: %f\n", data);
    } else {
...
int main() {
  ...
  tasks_init_task(ads1115_reader, 4, NULL);
  tasks_init_task(ads1115_writer, 3, NULL);
