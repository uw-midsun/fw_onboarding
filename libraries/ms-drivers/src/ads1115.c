...
StatusCode ads1115_init(ADS1115_Config *config, ADS1115_Address i2c_addr, GpioAddress *ready_pin) {
  ...
  uint16_t raw_low = 0;
  i2c_write_reg(config->i2c_port, i2c_addr, ADS1115_REG_LO_THRESH, (uint8_t *)(&raw_low), 2);

  /* Configure higher threshold to be 1.5V */
  uint16_t raw_high = 24000;
  i2c_write_reg(config->i2c_port, i2c_addr, ADS1115_REG_HI_THRESH, (uint8_t *)(&raw_high), 2);
  /* ---------------------- FW103 END ---------------------- */
...
StatusCode ads1115_read_converted(ADS1115_Config *config, ADS1115_Channel channel, float *reading) {
  ...
  int16_t raw_value = 0;
  ads1115_read_raw(config, channel, &raw_value);
