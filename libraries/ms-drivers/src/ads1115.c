...
StatusCode ads1115_select_channel(ADS1115_Config *config, ADS1115_Channel channel) {
  ...
  cmd |= 0x0000U;
  cmd |= (0x4000 | (uint16_t)channel << 12);
...
StatusCode ads1115_read_raw(ADS1115_Config *config, ADS1115_Channel channel, int16_t *reading) {
  ...
  status_ok_or_return(i2c_read_reg(config->i2c_port, config->i2c_addr, ADS1115_REG_CONVERSION, (uint8_t *)reading, 2));
  /* ---------------------- FW103 END ---------------------- */
  return STATUS_CODE_OK;
}
...
StatusCode ads1115_read_converted(ADS1115_Config *config, ADS1115_Channel channel, float *reading) {
  ...
  int16_t raw_value;
  status_ok_or_return(ads1115_read_raw(config, channel, &raw_value));
